...
export async function POST(
  ...
  const {preview_image, ...fields} = formData as unknown as Omit<PostRequest, "preview_image_url"> & {preview_image ?: File}
  ...
  if (preview_image) {
    ...
    const {data: uploadData, error} = await supabase.storage.from('blog-image').upload(fileName, preview_image, {
    ...
    if (uploadData?.path) {
      const { data } = await supabase.storage.from("blog-image").getPublicUrl(uploadData.path)
      preview_image_url = data.publicUrl;
    }
  }

  const { data } = await supabase.from("Post").insert([{...fields, preview_image_url}]).select()
